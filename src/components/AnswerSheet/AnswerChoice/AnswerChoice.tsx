...
function AnswerChoice(props: {
  ...
  const submitAnswerHandler = () => {
    ...
    let hintClassArray: string[] = props.question.map((q, i) =>
      q === state.answer[i]
        ? "exact-match"
        : state.answer.includes(q)
        ? "match"
        : "no-match"
    );
    hintClassArray = hintClassArray
      .filter((cl) => cl === "exact-match")
      .concat(
        hintClassArray
          .filter((cl) => cl === "match")
  ...
  return (
    <div
      className={"answer-choice " + (props.isActive ? "active" : "inactive")}
    >
      ...
      <div className="tick-container">
        {state.answer.every((ans: string) => ans !== "") &&
          props.isActive &&
          tick}
      </div>
      <div className="answer-hint-container">
        {Array.from(Array(4)).map((_, i) => (
          <span
            className={
              "answer-hint " +
              (!state.hintClasses[i] || state.hintClasses[i] === "match"
                ? ""
                : state.hintClasses[i])
            }
            key={i}
